...
?>
	<header class="page-header">
		...
		<h1 class="page-title">Alteração de Password:</h1>
			<p>Caro <?php echo $utilizador?>, aqui pode alterar a sua Password<p>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    ...
    $sqlinsert = "UPDATE user SET pass='{$password2}' WHERE user = '$utilizador' ";
...
else // Se não for uma requisição POST, mostra o formulário
{
   /* $id = $_GET['idNumber'];*/
    ...
    $sql = mysql_query("SELECT * FROM user WHERE user = '$utilizador'");
		if($sql === FALSE) {
