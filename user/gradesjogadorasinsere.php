...
if(isset($_POST['novo'])) {
	...
	$autd = "INSERT INTO setor (idsetor, nomesetor) VALUES ('$idsetor', '$nome')";
    mysql_query($autd,$ligacao);
}
?>
...
<section id="content" class="container clearfix">
	...
	<h2 class="slogan align-center"> Notas da <?php echo $nome; ?> </h2>
	<p> Aqui estão listadas todas as notas de <b><?php echo $nome; ?></b> </p>
...
if ($nota > 9) {
    echo "<td>";
	echo "<font color='green'><b>$nota</b></font>";
	echo "</td>";
} else {
    echo "<td>";
	echo "<font color='red'><b>$nota</b></font>";
	echo "</td>";
}
...
<?php
//Querys dos Serviços			
			$aut="SELECT MIN(classification) AS piornota FROM academic_path WHERE id_athlete='$variavel'";
			$resultado = mysql_query($aut,$ligacao);
			$row = mysql_fetch_assoc($resultado);
			$valorb = $row["piornota"];
			echo "<b>Nota mais baixa</b> ".$valorb." ";
			
			$aut2="SELECT MAX(classification) AS melhornota FROM academic_path WHERE id_athlete='$variavel'";
			$resultado = mysql_query($aut2,$ligacao);
			$row = mysql_fetch_assoc($resultado);
			$valorc = $row["melhornota"];
			echo "<b>Nota mais alta</b> ".$valorc."  </br>";
			
			$aut3="SELECT ROUND( AVG(classification),1) AS medianotas FROM academic_path WHERE id_athlete='$variavel'";
			$resultado = mysql_query($aut3,$ligacao);
			$row = mysql_fetch_assoc($resultado);
			$valora = $row["medianotas"];
			echo "<b>Media das tuas notas: </b> ".$valora." ";
?>
</center>
<!-- FIM NOTAS  -->
